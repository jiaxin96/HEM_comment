	...
	with gzip.open(output_path + 'review_id.txt.gz', 'w') as fout_id:
		with gzip.open(review_file, 'r') as g:
			...
			for l in g:
				...
				for term in summary.strip().split(' '):
					if term in word_map:
						fout_text.write(str.encode(word_map[term] + ' '))
						count_words += 1
				for term in review_text.strip().split(' '):
					if term in word_map:
						fout_text.write(str.encode(word_map[term] + ' '))
				...
				if count_words > 0:
					fout_text.write(str.encode('\n'))
					fout_u_p.write(str.encode(user_map[user] + ' ' + product_map[product] + '\n'))
					fout_id.write(str.encode('line_' + str(index) + '\n'))
				index += 1
