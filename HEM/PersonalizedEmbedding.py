...
def get_fs_from_words(model, word_idxs, reuse, scope=None):
	'''
	word_idxs <--- query_word_idxs
	shape: [batch_size, max_query_length]
	'''
...
# get_query_embedding(model, query_word_idxs, None)
...
def UQP_nce_loss(model, user_idxs, query_word_idxs, product_idxs, review_idxs,											
	...
	if model.need_review:										
		...
		# 这个loss是user language model和item language model的loss
